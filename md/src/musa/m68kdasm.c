...
static uint dasm_read_imm_8(uint advance)
{
	...
	if (g_rawop)
		...
		result = m68k_read_memory_8(g_cpu_pc & g_address_mask) & 0xff;
	g_cpu_pc += advance;
	return result;
}
...
static uint dasm_read_imm_16(uint advance)
{
	...
	if (g_rawop)
		...
		result = m68k_read_memory_16(g_cpu_pc & g_address_mask) & 0xffff;
	g_cpu_pc += advance;
	return result;
}
...
static uint dasm_read_imm_32(uint advance)
{
	...
	if (g_rawop)
		...
		result = m68k_read_memory_32(g_cpu_pc & g_address_mask) & 0xffffffff;
	g_cpu_pc += advance;
	return result;
}
